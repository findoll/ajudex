  // Configuração AWS S3 para upload de fotos
  ...
  // Verificação de conectividade S3 (opcional)
  async function testS3Connection() {
    ...
    try {
      ...
      console.log('✅ Conectividade S3 verificada');
      return true;
    } catch (error) {
  ...
  const uploadS3 = multer({
    storage: multerS3({
      ...
      metadata: function (req, file, cb) {
        console.log('🔧 Multer-S3 metadata callback:', {
      ...
      key: function (req, file, cb) {
        ...
        console.log('🔧 Multer-S3 key callback gerando:', filename, 'para usuário ID:', userId);
        cb(null, filename);
      },
    ...
    fileFilter: (req, file, cb) => {
      ...
      if (file.mimetype.startsWith("image/")) {
        ...
        console.log('❌ Arquivo rejeitado pelo filtro - não é imagem');
